...
class CashCollectionController extends Controller
{
    ...
    {
        if(Auth::user()){
            $cashCollections = CashCollection::select("cash_collections.*","branches.branch_name")
                ->join("branches","branches.branch_id","=","cash_collections.branch_id")
                ->where(['cash_collections.clinic_id' => $request->clinic_id,"isDelete" => 0])
                ->when($request->branch_id, function ($query, $branchId) {
                    return $query->where('cash_collections.branch_id', $branchId);
                })
            ...
            if (!$cashCollections->isEmpty()) {
                ...
                foreach($cashCollections as $cashCollection){
                    ...
                    $data[] = array(
                        ...
                        'branch_name' => $cashCollection->branch_name,
    ...
    public function destroy(Request $request)
    {
        if(Auth::user()){
            ...
            $cashLedger = CashLedger::where(["clinic_id" => $collection->clinic_id,"branch_id" => $collection->branch_id])->orderBy("id","desc")->first();
            $op_amt = $cashLedger->cl_amt ?? 0;
    ...
    public function CashCollectionReport(Request $request){
        if(Auth::user()){
            ...
            if(!empty($request->branch_id) && $request->branch_id != ""){
                $branchIds = implode(',', $request->branch_id);
                $OrderWhere .= " and branch_id in ($branchIds)";
                $ExpensesWhere .= " and branch_id in ($branchIds)";
                $CollectionWhere .= " and branch_id in ($branchIds)";
            }
    ...
    /*        $result = CashCollection::selectRaw('IFNULL(SUM(amount),0) as AdminCashCollection,
                    IFNULL((SELECT SUM(amount) FROM order_payment_detail WHERE '.$OrderWhere.' AND payment_mode=1),0) as patientCashCollection,
                    IFNULL((SELECT SUM(amount) FROM cash_expenses WHERE '.$ExpensesWhere.'),0) as CashExpense,cash_collections.branch_id,branches.branch_name,cash_collections.clinic_id')
                ->join("branches","branches.branch_id","=","cash_collections.branch_id")
                ->where(['cash_collections.clinic_id' => $request->clinic_id])
                ->when($request->branch_id, function ($query) use ($request) {
					$query->whereIn("cash_collections.branch_id",$request->branch_id);
				})
				->when($request->fromDate, function ($query) use ($request) {
						$query->where(DB::raw("DATE_FORMAT(collection_date,'%Y-%m-%d')"),'>=',DB::raw("DATE_FORMAT('".date('Y-m-d',strtotime($request->fromDate))."','%Y-%m-%d')"));
			    })
			    ->when($request->toDate, function ($query) use ($request) {
						$query->where(DB::raw("DATE_FORMAT(collection_date,'%Y-%m-%d')"),'<=',DB::raw("DATE_FORMAT('".date('Y-m-d',strtotime($request->toDate))."','%Y-%m-%d')"));
			    })
			    ->when($request->selected_date, function ($query) use ($request) {
						$query->where(DB::raw("DATE_FORMAT(collection_date,'%Y-%m-%d')"),'=',DB::raw("DATE_FORMAT('".date('Y-m-d',strtotime($request->selected_date))."','%Y-%m-%d')"));
			    })
			    ->when($request->month, function ($query) use ($request) {
					$query->where(DB::raw("MONTH(collection_date)"),'=',$request->month);
				})
    			->when($request->year, function ($query) use ($request) {
					$query->where(DB::raw("YEAR(collection_date)"),'=',$request->year);
				})
				->groupBy('cash_collections.branch_id')
                ->get();*/
        ...
        if(Auth::user()){
            /*$data = DB::table('cash_ledgers')
                ->select(
                    ...
                    DB::raw("
                        CASE 
                            WHEN order_id != 0 OR order_payment_detail_id != 0 THEN 'Patient Payment'
                            WHEN cash_expense_id != 0 THEN 'Cash Expense'
                            WHEN cash_collection_id != 0 THEN 'Cash Collection'
                            ELSE 'Reset Opening'
                        END AS Particular
                    ")
                )
                ...
                ->select(
                    ...
                    "branch_id",
                    DB::raw("(select branches.branch_name from branches where branches.branch_id=cash_ledgers.branch_id) as branch_name"),
                    /*DB::raw("SUM(CASE WHEN order_id > 0 THEN cr_amt ELSE 0 END  +
                        CASE 
                            WHEN cash_collection_id > 0 THEN cr_amt 
                            ELSE 0 
                        END +
                        CASE 
                            WHEN cash_expense_id > 0 THEN cr_amt 
                            ELSE 0 
                    ...
                    /*DB::raw("(SUM(CASE WHEN order_id > 0 THEN cr_amt ELSE 0 END) - 
                     SUM(CASE WHEN cash_expense_id > 0 THEN dr_amt ELSE 0 END) - 
                     SUM(CASE WHEN cash_collection_id > 0 THEN dr_amt ELSE 0 END)) AS cash_on_hand")*/
                    //  DB::raw("(CASE WHEN cash_collection_id=0 and order_id=0 and cash_expense_id=0 and cash_collection_id=0 THEN op_amt ELSE 
                    // (
                    //         SELECT op_amt 
                    ...
                    DB::raw("(
                            ...
                            AND cl.branch_id = cash_ledgers.branch_id 
                            AND DATE(cl.created_at) = DATE(cash_ledgers.created_at)
                            ORDER BY cl.id DESC 
                            LIMIT 1
                        ) AS cash_on_hand")
                    //DB::raw("SUM(CASE WHEN cash_collection_id=0 and order_id=0 and cash_expense_id=0 and cash_collection_id=0 THEN op_amt ELSE 0 END) AS cash_on_hand")
                )
                ->where('clinic_id', $request->clinic_id)
                ->when($request->branch_id, function ($query) use ($request) {
					$query->where("branch_id",$request->branch_id);
