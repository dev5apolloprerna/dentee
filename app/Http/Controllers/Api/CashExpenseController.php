...
class CashExpenseController extends Controller
{
    ...
    {
        if(Auth::user()){
            $cashExpenses = CashExpense::select("cash_expenses.*","branches.branch_name")
                ->join("branches","branches.branch_id","=","cash_expenses.branch_id")
                //->where(["cash_expenses.clinic_id" => $request->clinic_id,"cash_expenses.branch_id" => $request->branch_id,"isDelete" => 0])
                ->where(["cash_expenses.clinic_id" => $request->clinic_id,"isDelete" => 0])
                ->when($request->branch_id, function ($query) use ($request) {
				 	$query->where("cash_expenses.branch_id",$request->branch_id);
				})
				...
				->orderBy('cash_expenses.branch_id','asc')
            ...
            if (!$cashExpenses->isEmpty()) {
                ...
                foreach($cashExpenses as $cashExpense){
                    $data[] = array(
                        ...
                        'branch_id' => $cashExpense->branch_id,
                        'branch_name' => $cashExpense->branch_name,
                ...
                if($pdffile == 1 && !empty($data)){
                    /*usort($data, function($a, $b) {
                        // First compare by branch_id
                        if ($a['branch_id'] != $b['branch_id']) {
                            return $a['branch_id'] - $b['branch_id'];
                        }
                        
                        // If branch_id is the same, compare by expense_date
                        $dateA = DateTime::createFromFormat('d-m-Y', $a['expense_date']);
                        $dateB = DateTime::createFromFormat('d-m-Y', $b['expense_date']);
                        return $dateA <=> $dateB;
                    });*/
    ...
    public function destroy(Request $request)
    {
        if(Auth::user()){
            ...
            $cashLedger = CashLedger::where(["clinic_id" => $expense->clinic_id,"branch_id" => $expense->branch_id])->orderBy("id","desc")->first();
            $op_amt = $cashLedger->cl_amt ?? 0;
    ...
    public function CashExpensesVoucher(Request $request){
        ...
        if(Auth::user()){
            $cashExpense = CashExpense::select("cash_expenses.*","branches.branch_name")
                ->join("branches","branches.branch_id","=","cash_expenses.branch_id")
