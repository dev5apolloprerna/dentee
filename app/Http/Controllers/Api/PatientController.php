	...
	public function patientRegister(Request $request)
	{
		...
		if (Auth::user()) {
			...
			if (!empty($existPatient)) {
				...
				$wid = "28849"; // template id
				$PatientName = $request->name_prefix . " " . $request->name;
				$bodyValues = [
					"1" => $PatientName
				];
				
				$whatsappService->sendText($request->mobile_no, $wid, $bodyValues);
...
// 				$status = $users->sendWhatsappMessage($request->mobile_no, $key, $msg, $billFile = "");
		...
		if (Auth::user()) {
			...
			if (count($treatmentData) != 0) {
				...
				if ($pdffile == 1) {
					...
					//                 ];
					$pdf = PDF::loadView('patient_report', [
						'treatmentData' => $arr,
					...
					if ($_SERVER['SERVER_NAME'] == "127.0.0.1") {
						...
						$pdf->save(public_path('../../vgdcapp.vrajdentalclinic.com/assets/patient_report/')  . $fileName . '.pdf');
					}
					...
					if ($whatsappfile == 1) {
						...
						$mobileNo = "7046673769";//$currentUser->mobile_no;
				        // $status = $users->sendWhatsappMessage($mobileNo, $key, $msg, $treatmentListFile);
                        $whatsappService = new AuthkeyWhatsAppService();
        				$wid = "28598"; // template id
        				// $PatientName = $request->name_prefix . " " . $request->name;
        				// $bodyValues = [
        				// 	"1" => $PatientName
        				// ];
        				// $File = "https://vgdcapp.vrajdentalclinic.com/assets/patient_report/".$File;
        				$bodyValues=[
        				    "1" => $fileName
    				    ];
	...
	public function PatientExport(Request $request, $clinic_id, $branch_id)
	{
// 		if (!Auth::user()) {
...
// 		$branch_id = $request->branch_id;
		...
		// Conditionally filter according to the flag
		$currentDate = date('Y-m-d');

		/*if ($flag == 1) {
			// Recently registered
			if (!empty($request->fromDate) && !empty($request->toDate)) {
				$query->whereBetween(DB::raw("DATE_FORMAT(patients.created_at,'%Y-%m-%d')"), [$request->fromDate, $request->toDate]);
			} else {
				$query->where(DB::raw("(DATE_FORMAT(patients.created_at,'%d-%m-%Y'))"), date('d-m-Y'));
			}
		} elseif ($flag == 2) {
			// Today's appointments
			$query->join('appointments', 'appointments.patient_id', '=', 'patients.patient_id')
				->where('appointments.appointment_date', $currentDate);
		} elseif ($flag == 3) {
			// Recently visited (suggested_treatments or order_master)
			$query->leftJoin('suggested_treatments', 'suggested_treatments.patient_id', '=', 'patients.patient_id')
				->leftJoin('order_master', 'order_master.patient_id', '=', 'patients.patient_id')
				->where(function ($q) use ($request) {
					$q->whereBetween(DB::raw("DATE_FORMAT(suggested_treatments.treatment_date,'%Y-%m-%d')"), [$request->fromDate, $request->toDate])
						->orWhereBetween(DB::raw("DATE_FORMAT(order_master.created_at,'%Y-%m-%d')"), [$request->fromDate, $request->toDate]);
				})
				->groupBy('patients.patient_id');
		}*/
		...
		$PatientList = $query->orderBy('patients.patient_id', 'desc')->get();

		return Excel::download(new PatientExport($PatientList), 'patient-list.xlsx');
	}
